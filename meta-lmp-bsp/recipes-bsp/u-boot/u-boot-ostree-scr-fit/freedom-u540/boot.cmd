...
setenv bootcmd_otenv 'run bootcmd_resetvars; ext4load ${devtype} ${devnum}:4 ${scriptaddr} /boot/loader/uEnv.txt; env import -t ${scriptaddr} ${filesize} kernel_image bootargs kernel_image2 bootargs2'
setenv bootcmd_load_f 'ext4load ${devtype} ${devnum}:4 ${ramdisk_addr_r} "/boot"${kernel_image}'
...
if test ! -e ${devtype} ${devnum}:3 uboot.env; then saveenv; fi
